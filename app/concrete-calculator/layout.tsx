import type { Metadata } from "next";

// The calculator page itself is a client component and cannot export metadata,
// so it lives here. Without it every calculator inherited the site-default
// title and emitted no canonical — 13 pages competing on one duplicate title.
export const metadata: Metadata = {
  title: "Concrete Calculator: Bags & Cubic Yards",
  description:
    "Work out bags or cubic yards for slabs, footings, and post holes, plus the tools you need to pour it. Enter dimensions, get a full list.",
  alternates: { canonical: "https://www.buildguiders.com/concrete-calculator" },
  openGraph: {
    title: "Concrete Calculator: Bags & Cubic Yards | BuildGuiders",
    description:
      "Work out bags or cubic yards for slabs, footings, and post holes, plus the tools you need to pour it. Enter dimensions, get a full list.",
    // Must match the canonical: the inherited og:url pointed every calculator
    // at the homepage.
    url: "https://www.buildguiders.com/concrete-calculator",
    type: "website",
    siteName: "BuildGuiders",
    images: [{ url: "https://www.buildguiders.com/og-default.png", width: 1200, height: 630 }],
  },
};

export default function Layout({ children }: { children: React.ReactNode }) {
  return children;
}
